...
export default function Home() {
    ...
    try {
      ...
      const resp = await apiFetch(
        ...
        body: JSON.stringify({
          ...
          source_title: title || "AI Video Lecture Notes",
          source_url: input.kind === "url" ? input.source : "",
        }),
        },
        accessToken,
      );
      ...
      const pdfBytes = await generatePdfFromMarkdown({
        ...
        branding: {
          brandName: "LectureLens",
          accentColor: "#0f766e",
          footerText: "Generated by LectureLens",
        },
      });
  ...
  const studyPack = buildStudyPack({
    transcript: transcriptText,
    notesMarkdown,
    chapters: extractChapters(transcriptText),
    style: summaryStyle,
    outputLanguage,
    sourceTitle: title,
  });
  ...
  function submitChatQuestion() {
    ...
    const result = buildChatAnswer(question, transcriptText, notesMarkdown, studyPack.sections);
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <label className="block">
                    ...
                    >
                      ...
                      <option value="mind_map">Mind Map Outline</option>
                      <option value="research_abstract">Research Abstract</option>
                    </select>
                  </label>
              ...
              <AnimatePresence>
                {job && job.status !== "idle" ? (
                  <div className="mt-5 space-y-6">
                    <div className="rounded-xl border border-[var(--border)] bg-gradient-to-br from-teal-500/10 via-[var(--card)] to-transparent p-4">
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        <div>
                          <div className="text-sm font-medium">Summary Card</div>
                          <div className="text-xs text-[var(--muted)] mt-1">
                            {studyPack.style.replace(/_/g, " ")} - {studyPack.outputLanguage}
                          </div>
                        </div>
                        <div className="text-xs text-[var(--muted)]">{studyPack.keyConcepts.length} key concepts</div>
                      </div>
                      <p className="mt-3 text-sm leading-relaxed">{studyPack.tldr}</p>
                      <div className="mt-4 flex flex-wrap gap-2">
                        {studyPack.keyConcepts.slice(0, 5).map((concept) => (
                          <span key={concept} className="px-3 py-1 rounded-full border border-[var(--border)] text-xs bg-white/5">
                            {concept}
                          </span>
                        ))}
                      </div>
                      ...
                      <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                          <div>
                            ...
                            <div className="text-xs text-[var(--muted)] mt-1">Auto-generated practice questions</div>
                          </div>
                          <button
                            className="px-3 py-2 rounded-xl border border-[var(--border)] hover:bg-white/10 transition text-sm"
                            disabled={!studyPack.quizQuestions.length}
                            onClick={() => {
                              const blob = new Blob([JSON.stringify(studyPack.quizQuestions, null, 2)], {
                                type: "application/json;charset=utf-8",
                              });
                              const a = document.createElement("a");
                              a.href = URL.createObjectURL(blob);
                        ...
                        <div className="mt-4 space-y-3">
                          {studyPack.quizQuestions.map((question, index) => (
                            <div key={`${question.question}-${index}`} className="rounded-xl border border-[var(--border)] bg-black/5 dark:bg-white/5 p-3">
                              <div className="font-medium text-sm">{index + 1}. {question.question}</div>
                              <div className="mt-2 grid gap-2 text-sm">
                                {question.options.map((option, optionIndex) => (
                                  <div key={option} className={`rounded-lg px-3 py-2 border ${optionIndex === question.answerIndex ? "border-teal-500/50 bg-teal-500/10" : "border-[var(--border)]"}`}>
                                    {option}
                                  </div>
                                ))}
                              </div>
                              <div className="mt-2 text-xs text-[var(--muted)]">{question.explanation}</div>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                          <div>
                            ...
                            <div className="text-xs text-[var(--muted)] mt-1">Study and export to Anki</div>
                          </div>
                          <button
                            className="px-3 py-2 rounded-xl border border-[var(--border)] hover:bg-white/10 transition text-sm"
                            disabled={!studyPack.flashcards.length}
                            onClick={() => {
                              const blob = new Blob([JSON.stringify(studyPack.flashcards, null, 2)], {
                                type: "application/json;charset=utf-8",
                              });
                              const a = document.createElement("a");
                              a.href = URL.createObjectURL(blob);
                              a.download = `${title}_flashcards.json`;
                              a.click();
                            }}
                          >
                            Export Anki
                        ...
                        <div className="mt-4 space-y-3 max-h-[420px] overflow-auto pr-1">
                          {studyPack.flashcards.map((card, index) => (
                            <div key={`${card.front}-${index}`} className="rounded-xl border border-[var(--border)] bg-black/5 dark:bg-white/5 p-3">
                              <div className="text-xs text-[var(--muted)]">Front</div>
                              <div className="font-medium">{card.front}</div>
                              <div className="mt-3 text-xs text-[var(--muted)]">Back</div>
                    ...
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                      <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
                        <div className="text-sm font-medium">Chat with your video</div>
                        <p className="text-xs text-[var(--muted)] mt-1">Grounded answers from the transcript and notes.</p>
                        <div className="mt-4 flex gap-2">
                          <input
                            value={chatQuestion}
                            onChange={(e) => setChatQuestion(e.target.value)}
                            placeholder="Ask about examples, definitions, or conclusions..."
                            className="flex-1 rounded-xl border border-[var(--border)] bg-transparent px-4 py-3 outline-none focus:ring-2 focus:ring-teal-500/30"
                          />
                          <button
                            onClick={submitChatQuestion}
                            disabled={!chatQuestion.trim() || !transcriptText}
                            className="px-4 py-3 rounded-xl bg-teal-600 hover:bg-teal-600/90 text-white transition disabled:opacity-60"
                          >
                            Ask
                          </button>
                        </div>
                        <div className="mt-4 rounded-xl border border-[var(--border)] bg-black/5 dark:bg-white/5 p-3 text-sm leading-relaxed min-h-24">
                          {chatAnswer || "Ask a question to get a grounded answer from the transcript."}
                        </div>
                        {chatCitations.length > 0 && (
                          <div className="mt-3 space-y-2">
                            {chatCitations.map((citation) => (
                              <div key={`${citation.source}-${citation.index}`} className="text-xs text-[var(--muted)] rounded-lg border border-[var(--border)] px-3 py-2">
                                {citation.source} #{citation.index + 1}: {citation.text}
                              </div>
                            ))}
                          </div>
                        )}
                      ...
                      <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-4">
                        ...
                        <p className="text-xs text-[var(--muted)] mt-1">Export-ready payloads for Notion, Google Docs, and Obsidian.</p>
                        <div className="mt-4 flex flex-wrap gap-2">
                          <button
                            className="px-3 py-2 rounded-xl border border-[var(--border)] hover:bg-white/10 transition text-sm"
                            disabled={!notesMarkdown}
                            onClick={() => navigator.clipboard.writeText(JSON.stringify(buildNotionExport(studyPack), null, 2))}
                          >
                            Notion
                          </button>
                          <button
                            className="px-3 py-2 rounded-xl border border-[var(--border)] hover:bg-white/10 transition text-sm"
                            disabled={!notesMarkdown}
                            onClick={() => navigator.clipboard.writeText(JSON.stringify(buildGoogleDocsExport(studyPack), null, 2))}
                          >
                            Google Docs
                          </button>
                          <button
                            className="px-3 py-2 rounded-xl border border-[var(--border)] hover:bg-white/10 transition text-sm"
                            disabled={!notesMarkdown}
                            onClick={() => navigator.clipboard.writeText(buildObsidianMarkdown(studyPack))}
                          >
                            Obsidian Markdown
                          </button>
                        </div>
